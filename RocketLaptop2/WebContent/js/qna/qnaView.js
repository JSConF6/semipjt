			...
			success:function(rdata){
			 ...
			 if(rdata.qnalist.length>0){
				   output += '<li class="comment_tab_item ' +  red1 + '" >'
				    ...
				    output='';
					$(rdata.qnalist).each(function(){//<------------array로 끄집어 낼 수 있게
...
function updateForm(num){//num : 수정할 댓글 글번호
	...
	//'.btn_register' 영역에 수정할 글 번호를 속성 'data-id'에 나타내고 
	//클래스 'update'를 추가하며 등록을 수정완료로 바꿉니다.
...
function replyform(num,lev,seq,ref){
	//댓글달기 폼이 열려있다는 것은 다른 폼이 열려있지 않은 경우 입니다.
...
$(function(){
	...
	$('ul+.CommentWriter .btn_register').click(function(){
		...
		$('.comment_inbox_text').val('');//textarea 초기화
		$('.comment_inbox_count').text('');//입력한 글 카운트 초기화
	})//('.btn_register').click(function() end
			
	//더보기를 클릭하면 수정과 삭제 영역이 나타나고 다시 클릭하면 사라져요-toggle()이용	매개변수 3개!!!!!	
	$(".comment_list").on('click', '.comment_tool_button', function(){
		...
		if($('.comment_list+.CommentWriter').css('display')=='block'){
			...
			//더보기를 여러개 선택하더라고 최종 선택한 더보기 한개만 보이도롭 합니다.
	...
	$('.CommentBox').on('click','.update_cancel',function(){
		...
		$(selector +' .CommentWriter').remove(); //<========= 후손 앞에 공백 필수 추가!!
